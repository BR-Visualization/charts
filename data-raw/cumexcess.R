## code to prepare `cumexcess_plot` dataset goes here

load("~/brcharts/data-raw/manuscript_example.rda")

cumexcess <- manuscript_example %>%
  filter(subgroup == "Overall") %>%
  select(trt_diff_lbl, treat_code, treatment, study_duration, eventtimeunit,
         eventtime, smalln, outcome, trt_diff_f)

cumexcess <- cumexcess %>% rename(eff_diff_lbl = trt_diff_lbl,
