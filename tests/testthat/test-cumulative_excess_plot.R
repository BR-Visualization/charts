...
cumexcess_table <- data.frame(
  ...
  n = c(
    0, 50, 78, 117, 156, 194, 230, 0, 34, 71, 108, 156, 192, 232, 0, 7, 18,
...
cumexcess_table1 <- cumexcess_table %>% select(
  effect, outcome,
  obsv_duration,
  eff_code
)
...
cumexcess_plot1 <- cumexcess_plot %>% select(
  eventtime, outcome, obsv_duration,
  eff_diff_lbl, obsv_unit
)
...
test_that(
  "gensurv_table() expects an error when df_table() and df_plot() have
misnamed variables",
  {
    expect_error(
      gensurv_combined(
