...
check_effects_table <- function(df) {
  tags_ui <- paste0("<ol>",
                    check_feature(data = df, feature = "Factor",
                                  plots = c("value_tree", "forest", "contour", "tradeoff"),
                                  func = is.character, na_check = T,
                                  values = c("Benefit", "Risk")),

                    check_feature(data = df, feature = "Grouped_Outcome", plots = "value_tree",
                                  func = is.character, na_check = T),

                    check_feature(data = df, feature = "Outcome",
                                  plots = c("value_tree", "forest", "tradeoff", "contour"),
                                  func = is.character, na_check = T,
                                  check_unique = c("Factor","Grouped_Outcome",
                                                   "Statistics", "Outcome_Status",
                                                   "Type")),

                    check_feature(data = df, feature = "Statistics", plots = "value_tree",
                                  func = is.character, na_check = T),

                    check_feature(data = df, feature = "Outcome_Status", plots = "value_tree",
                                  is.character, na_check = T,
                                  values = c("Identified", "Potential")),

                    check_feature(data = df, feature = "Filter",
                                  plots = c("forest", "contour", "tradeoff"),
                                  func = is.character, na_check = T),

                    check_feature(data = df, feature = "Category",
                                  plots = c("forest", "contour", "tradeoff"),
                                  func = is.character, na_check = T),

                    check_feature(data = df, feature = "Type",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.character, na_check = T,
                                  values = c("Binary", "Continuous")),

                    check_feature(data = df, feature = "Rate_Type",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.character),

                    check_feature(data = df, feature = "Mean1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.numeric),

                    check_feature(data = df, feature = "Mean2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.numeric),

                    check_feature(data = df, feature = "Prop1",
                                  plots = c("contour", "forest", "tradeoff"), func = is.numeric,
                                  check_range = c(0,1)),

                    check_feature(data = df, feature = "Prop2",
                                  plots = c("contour", "forest", "tradeoff"), func = is.numeric,
                                  check_range = c(0,1)),

                    check_feature(data = df, feature = "N1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.integer, check_positive = T),

                    check_feature(data = df, feature = "N2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.integer, check_positive = T),

                    check_feature(data = df, feature = "100PYAR1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "100PYAR2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "IncRate1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "IncRate2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "100PEY1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "100PEY2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "EventRate1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "EventRate2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "Sd1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "Sd2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.numeric, check_positive = T),

                    check_feature(data = df, feature = "Trt1",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func =  is.character,
                                  na_check = T),

                    check_feature(data = df, feature = "Trt2",
                                  plots = c("contour", "forest", "tradeoff"),
                                  func = is.character, na_check = T, check_same = T),

                    check_feature(data = df, feature = "Diff_LowerCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Diff_UpperCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Diff_IncRate_LowerCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Diff_IncRate_UpperCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Diff_EventRate_LowerCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Diff_EventRate_UpperCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "RelRisk_LowerCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "RelRisk_UpperCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "OddsRatio_LowerCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "OddsRatio_UpperCI",
                                  plots = c("forest", "tradeoff"), func = is.numeric),

                    check_feature(data = df, feature = "Drug_Status", plots = "tradeoff",
...
check_feature <- function(data, feature, plots, func, na_check, values,
  ...
  if (!(feature %in% colnames(data))) {
    error_msg <- glue("<li><span>Feature <b>{feature}</b> is missing in the ",
  ...
  # check if the feature is not empty
    ...
    error_msg <- glue("<li><span>Feature <b>{feature}</b> is empty : errors ",
  ...
  # check if the feature has the right type
    ...
    error_msg <- glue("<li><span>Feature <b>{feature}</b> must be of type ",
    ...
    if (!missing(na_check) && na_check == T) {
      if (sum(is.na(data[feature])) > 0) {
        error_msg <- glue("<li><span>Feature <b>{feature}</b> has missing values",
    ...
    if (!missing(values)) {
      if (!(all(data[[feature]] %in% values))) {
        error_msg <- paste(error_msg, glue("<li><span>Feature <b>{feature}</b>",
    ...
    if (!missing(check_range)) {
      ...
      if (!(b1 && b2)) {
        error_msg <- paste(error_msg, glue("<li><span>Feature <b>{feature}",
    ...
    if (!missing(check_unique) && length(check_unique) > 0) {
      ...
      if (all(check_unique %in% names(data)) == TRUE) {
        ...
        if (!all(apply(result[, 2:ncol(result)], 2,
                       function(a) length(unique(a))=="1") == TRUE)) {
          error_msg <- paste(error_msg, glue("<li><span>Feature ",
...
check_feature_string <- function(data, feature, plots, func, na_check, values,
  ...
  # check if the feature is available in the effects table
  if (!(feature %in% colnames(data))) {
    error_msg <- glue("Feature {feature} is missing in the ",
                      "effect table : errors occur in {toString(plots)}",
                      " plot(s);")
  }
  # check if the feature is not empty
  else if (sum(is.na(data[feature])) == nrow(data)) {
    error_msg <- glue("Feature {feature} is empty : errors ",
  ...
  # check if the feature has the right type
    ...
    typestr <- gsub("is.|\\(\\)", "", deparse(substitute(func())))
    error_msg <- glue("Feature {feature} must be of type ",
    ...
    if (!missing(na_check) && na_check == T) {
      if (sum(is.na(data[feature])) > 0) {
        error_msg <- glue("Feature {feature} has missing values",
                          " : errors occur in ",
    ...
    if (!missing(values)) {
      if (!(all(data[[feature]] %in% values))) {
        error_msg <- paste(error_msg, glue("Feature {feature}",
                                           " must have the following values **{toString(values)}**",
    ...
    if (!missing(check_unique) && length(check_unique) > 0) {
      ...
      if (all(check_unique %in% names(data)) == TRUE) {
        ...
        if (!all(apply(result[, 2:ncol(result)], 2,
                       function(a) length(unique(a))=="1") == TRUE)) {
          error_msg <- paste(error_msg, glue("Feature {feature} error :",
