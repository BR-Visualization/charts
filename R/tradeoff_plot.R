...
#' @param b1,b2,b3,b4,b5,b6,b7,b8,b9,b10 (`numeric`) specified benefitX
#' @param r1,r2,r3,r4,r5,r6,r7,r8,r9,r10 (`numeric`)
#'   specified risk tolerance at benefitX
...
generate_tradeoff_plot <- function(data, filter, category, benefit, risk,
  ...
  df_br <- prepare_tradeoff_data(data, filter, category, benefit, risk,
...
prepare_tradeoff_plot <- function(myplot, data, df_br, drug_status, filter, ci,
  ...
  if (filter != "None") {
    ...
    names(my_colors) <- c(t(outer(levels(as.factor(data$Trt1)),
                                  levels(as.factor(data$Category)), paste, sep =" : ")))
  } else {
  ...
  if (ci == "No") {
    ...
    myplot +
      geom_point(aes_string(x = "benefit", y = "risk", colour = "treatment",
                            shape = "treatment"),
                 data = df_br_status,
                 size = 3,
                 show.legend = TRUE
...
prepare_tradeoff_data <- function(data, filter, category, benefit, risk,
  ...
  error_msg <- paste0(
    check_feature_string(data = data, feature = "Outcome", plots = "tradeoff",
                         func = is.character, na_check = T,
                         check_unique = c("Factor", "Grouped_Outcome",
                                          "Statistics", "Outcome_Status",
                                          "Type")),

    check_feature_string(data = data, feature = "Factor",
                         plots = "tradeoff",
                         func = is.character, na_check = T,
                         values = c("Benefit", "Risk")),

    check_feature_string(data = data, feature = "Filter", plots = "tradeoff",
                         func = is.character, na_check = T),

    check_feature_string(data = data, feature = "Drug_Status",
                         plots = "tradeoff",
                         func = is.character, values = c("Approved", "Test")),

    check_feature_string(data = data, feature = "Category",
                         plots = "tradeoff",
                         func = is.character, na_check = T),

    check_feature_string(data = data, feature = "Trt1",
                         plots = "tradeoff", func =  is.character,
                         na_check = T),

    check_feature_string(data = data, feature = "Trt2",
                         plots = "tradeoff",
                         func = is.character, na_check = T,
                         check_same = T),

    check_feature_string(data = data, feature = "Type", plots = "tradeoff",
                         func = is.character, na_check = T,
  ...
  if (ci_method == "Supplied") {
    if (df_benefit$Type[1] == "Continuous") {
      ...
      error_msg <- paste0(
        check_feature_string(data = df_benefit, feature = "Mean1",
                             plots = "tradeoff",
                             func = is.numeric),

        check_feature_string(data = df_benefit, feature = "Mean2",
                             plots = "tradeoff",
                             func = is.numeric),
      ...
      error_msg <- paste0(
        check_feature_string(data = df_benefit, feature = "Prop1",
                             plots = "tradeoff", func = is.numeric,
                             check_range = c(0, 1)),

        check_feature_string(data = df_benefit, feature = "Prop2",
                             plots = "tradeoff", func = is.numeric,
                             check_range = c(0, 1)))
    ...
    if (df_benefit$Type[1] == "Continuous") {
      ...
      error_msg <- paste0(
        ...
        check_feature_string(data = df_benefit, feature = "N1",
                             plots = "tradeoff",
                             func = is.integer, check_positive = T),

        check_feature_string(data = df_benefit, feature = "N2",
                             plots = "tradeoff",
                             func =  is.integer, check_positive = T),

        check_feature_string(data = df_benefit, feature = "Sd1",
                             plots = "tradeoff",
                             func =  is.numeric, check_positive = T),

        check_feature_string(data = df_benefit, feature = "Sd2",
                             plots = "tradeoff",
                             func =  is.numeric, check_positive = T))
    ...
    if (type_risk == "Crude proportions") {
      ...
      error_msg <- paste0(
        check_feature_string(data = df_risk, feature = "Prop1",
                             plots = "tradeoff", func = is.numeric,
                             check_range = c(0, 1)),

        check_feature_string(data = df_risk, feature = "Prop2",
                             plots = "tradeoff", func = is.numeric,
                             check_range = c(0, 1)),

        check_feature_string(data = df_risk, feature = "N1",
                             plots = "tradeoff",
                             func = is.integer, check_positive = T),

        check_feature_string(data = df_risk, feature = "N2",
                             plots = "tradeoff",
                             func =  is.integer, check_positive = T))
